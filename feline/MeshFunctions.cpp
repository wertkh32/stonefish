	...
	*mesh = new Mesh(list, n*m*2);
	...
	for(int i=0;i<n-1;i++)
		for(int j=0;j<m-1;j++)
		{
			...
			(*mesh)->addElement(i * m + j, (i+1) * m + j, i * m + (j + 1), m * n + i * m + j, 50,0.1,10);
			(*mesh)->addElement((i+1) * m + (j+1), (i+1) * m + j, i * m + (j + 1), m * n + (i + 1) * m + (j + 1), 50,0.1,10);
			(*mesh)->addElement(m * n + i * m + j, m * n + (i+1) * m + j, m * n + (i+1) * m + (j+1), (i+1) * m + j, 50,0.1,10);
			(*mesh)->addElement(m * n + i * m + j, m * n + i * m + (j+1), m * n + (i+1) * m + (j+1), i * m + (j+1), 50,0.1,10);
